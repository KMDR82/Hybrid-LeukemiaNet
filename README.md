...
* `model.py`: Defines the proposed dual-stream architecture (Hybrid-LeukemiaNet), fusing CNN and ViT. It includes dynamic gating mechanisms to facilitate architectural ablation studies.
...
Due to GitHub's file size limits, the pre-trained weights for the proposed dual-stream model (approx. 350MB) and the dataset are hosted externally.
